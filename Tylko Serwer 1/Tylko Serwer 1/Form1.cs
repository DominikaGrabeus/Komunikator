    ...
    public partial class SerwerForm : Form
    {
        ...
        private void StartServer()
        {
            try
            {
                ...
                _serwerSocket.Bind(new IPEndPoint(IPAddress.Parse("192.168.43.131"), 8001));
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
              try
            {
                ...
                if (first == "TylkoNick")
                {
                    ...
                    if (!exist)
                        kitty.Add(new KittyClient(second, _clientSocket));
                    else
                    {
                ...
                if (first == "NowyUczestnikDane")
                    SaveToFile(text, @"C:\Users\Dominika Grabeus\Desktop\Tylko Serwer 1\Dane_osobowe.txt");
                if (first == "NowyUczestnikKomunikatora")
                    SaveToFile(text, @"C:\Users\Dominika Grabeus\Desktop\Tylko Serwer 1\Nick_haslo.txt");
                if (second == "DodajNowegoZnajomegoProsze")
                {
                    string[] liness = System.IO.File.ReadAllLines(@"C:\Users\Dominika Grabeus\Desktop\Tylko Serwer 1\Dane_osobowe.txt");
                    int j = 0;
                    bool tmp1 = true;
